...
#### This project I did focuses on trying to predict a driver's laptime on __ lap in the _____ Grand Prix using a linear regression model I created for that race. The first section is the pre-race predictions, which contains the prediction of the lap 30 time of Sergio Perez in every race of the 2024 F1 season, along with the Root Mean Squared Error of the model's testing predictions. The second section is the post-race analysis, which evaluates the predicted laptime of that race's lap and outputs the difference. It also displays informative plots summarizing the race.

(The default driver is Sergio Perez, default lap is 30. I still need to implement user interface in this page so that it takes the driver, lap, circuit you want to test and output the predicted lap time)

**The code I used for this project can be found in my GitHub Repository:** ***https://github.com/andrejcc04/Portfolio/blob/main/FormulaOne.Rmd***
